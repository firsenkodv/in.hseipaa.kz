...
<div class="cabinet_info__data">
    <div>
        @if($resume)
            <div class="r_price__label">Желаемый заработок</div>
            <div class="r_price">{{ price($item->price) }} {{ config('currency.currency.KZT') }}</div>
        @else
            <div class="flex">
                <div class="u_left">
                    <div class="site_avatar"
                         style="@if($creator?->avatar) background-image: url('{{ asset(intervention('48x48', $creator->avatar, '/managers/'.$creator->id.'/avatar/intervention')) }}');@endif width: 48px; height: 48px"></div>
                </div>
                <div class="u_right">
                    <div class="_name"
                         title="{{ $creator?->legalEntity ? $creator->company : $creator?->username }}">{{ $creator?->legalEntity ? $creator->company : $creator?->username }}</div>
                    <div class="_personal">{{ $creator?->UserHuman?->title }}</div>
                </div>
        ...
        @if($hasTarif)
            <div class="blockYourManeger__bottom">
                ...
                <div class="_bottom_flex">
                    <div class="_bottom_left">
                        ...
                        <div class="_bottom_icon_text">
                            @if($contact->phone)
                                <a href="tel:{{ $contact->phone }}">{{ format_phone($contact->phone) }}</a>
                            @endif
                        </div>
                    ...
                    <div class="_bottom_right">
                        @if($contact->whatsapp)
                            <a href="{{ checkWhatsapp($contact->whatsapp) }}" target="_blank">
                        ...
                        @endif
                        @if($contact->telegram)
                            <a href="{{ checkTelegram($contact->telegram) }}" target="_blank">
                ...
                <div class="_bottom_flex pad_b13_important">
                    <div class="_bottom_left _bottom_left_email">
                        ...
                        <div class="_bottom_icon_text">{{ $contact->email }}</div>
                    </div>
                </div>
                @if(!$resume)
                    <div class="_bottom_flex">
                        <div class="_bottom_left _bottom_left_email">
                            <div class="_bottom_icon_text"><span
                                    class="grey">Адрес:</span> {{ ($contact->address)?:'Не указан' }}</div>
                        </div>
                    </div>
                @endif
