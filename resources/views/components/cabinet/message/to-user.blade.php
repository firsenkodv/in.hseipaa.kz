...
<a  title="Написать сообщение" style="background:none;border:none;cursor:pointer;padding:0;color:inherit;" class="open-fancybox to_user_message" data-form="to_user_message" data-transfer='{"user_id": {{ $item->id }}, "action": "message"}'>
    ...
    <a  title="Заблокировать" style="background:none;border:none;cursor:pointer;padding:0;color:inherit;" class="{{ $isBlocked ? '' : 'open-fancybox' }} to_user_message_and_blocked {{ $isBlocked ? 'active' : '' }}" @if(!$isBlocked) data-form="to_user_message" data-transfer='{"user_id": {{ $item->id }}, "action": "blocked"}' @endif>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
